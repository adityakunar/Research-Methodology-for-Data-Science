...
data = readtable('./data/wdbc.data.csv');
...
function [snapshot, snap_labs] = get_constant_prior_snapshot(x, lab, sampling_ratio)
    ...
    for i = 1:num_classes
        ...
        disp(k)
        select_indexes = datasample(s, indexes, k, 'Replace',false);
...
function [snapshot, snap_labs] = get_random_snapshot(x, lab, sampling_ratio)
    ...
    disp(k);
    select_indexes = datasample(s, 1:size(lab,1), k, 'Replace',false);
