...
## Blind Analysis of models with and without Transfer Learning
...
For our analysis we  first pretend we only have model scores of when transfer learning was used and when it wasn't without any other additional information. And so, to check for a difference in mean scores, we can do an independent sample t test for which we need to make sure that our data is normally distributed. Here the null hypothesis is that there is no difference in mean scores between the two groups and the alternate hypthosis is that there is a difference in mean scores. 
...
### Non Parametric Test (Wilcox signed rank test.)
...
### Conclusion for the blind analysis

So based on our simple blindfolded analysis, *we know that transfer learning does have a significant impact on the scores of our model*. 
